...
def profile(common_user):
    profile = Profile.objects.create(user=common_user,first_name="test firstname",
                                     last_name="test lastname",bio="test bio",)
    return profile

@pytest.fixture
def task_example(profile):
    task = Task.objects.create(title="test task",status=True,author=profile,
            is_done=False,)
...
class TestTaskApi:
    ...
    def test_delete_task_authorized_status_201(self, api_client, common_user,task_example):
    ...
    def test_put_task_authorized_status_200(self, api_client, common_user,task_example):
        url = reverse("todo:api-v1:task-detail",args=[task_example.id])
        user = common_user
        data = {
            "title": "Go to the gym",
    ...
    def test_put_task_invalid_data_status_400(self, api_client, common_user,task_example):
        url = reverse("todo:api-v1:task-detail",args=[task_example.id])
        user = common_user
        invalid_data = {}
        api_client.force_authenticate(user=user)
